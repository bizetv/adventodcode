...
value 2 goes to bot 2`*/
	...
	for i := 0; i < len(inputArray); i++ {
		...
		case "value":
			...
			value,_ := strconv.Atoi(instructArr[1])
			AssignValue(mapVar, value, instructArr[5],inputArray)
...
func GiveValue(botList map[string]bot, from, toLow, toHigh string, inputArray []string){
	tmpBot := bot{from, 0, 0}
	low := botList[from].low
	high := botList[from].high
	//tmpBotLow := botList[toLow]
	//tmpBotHigh = botList[toHigh]
	if (low == 17 && high == 61){
		fmt.Println("TEEEEEEEEEEEEEEEEEST ", low, high, " bot== ", from)
	}
	if (toLow != "-1"){
		AssignValue(botList,low, toLow,inputArray)
	}
	if (toHigh != "-1"){
		AssignValue(botList, high, toHigh, inputArray)
	}
	botList[from] = tmpBot
	
}

func AssignValue(botList map[string]bot, value int, botName string, inputArray []string){
	tmpBot:= bot{}
	
	if (botList[botName].name == ""){
			//botList[botName] = bot{botName, value, 0}
		tmpBot.name = botName
	}else{
		tmpBot.name = botList[botName].name
		tmpBot.low = botList[botName].low
	}
	if (botList[botName].low == 0){
		tmpBot.low = value
	}else{
		if (botList[botName].low > value){
			tmpBot.high = botList[botName].low
			tmpBot.low = value
		}else{
			tmpBot.high = value
		}
	}
	botList[botName] = tmpBot
	fmt.Println(botList[botName])
	if ( botList[botName].low != 0 && botList[botName].high != 0){
		fmt.Println("TEST")
		for i := 0; i < len(inputArray); i++ {
			instructArr := strings.Split(inputArray[i], ` `)
			if (instructArr[1] == botName){
				switch instructArr[0]{
				case "bot":
					fmt.Println(instructArr)
					toLow := "-1"
					toHigh := "-1"
					if (instructArr[5] == "bot"){
					toLow = instructArr[6]
					}
					if (instructArr[10] == "bot"){
						toHigh = instructArr[11]
					}
					GiveValue(botList, instructArr[1],toLow,toHigh, inputArray)
					break
				}
			}
		}
