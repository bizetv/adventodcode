...
func main(){
	...
	var roomSet = string(`vxupkizork-sgmtkzoi-pkrrehkgt-zxgototm-644[kotgr]
...
shoewudys-tou-ixyffydw-478[uszty]`)
	var roomSetArray = strings.Split(roomSet, "\n")
	var nbRoomCheck = false
	var nbRoom = 0
	var alpha = "abcdefghijklmnopqrstuvwxyz"
	var alphaIndex = 0
	//var nbIteration = 0

	for _,room := range roomSetArray{
		//	nbIteration++
		nbRoomCheck = false
		roomCheck := room[ strings.Index(room, "[")+1:strings.Index(room,"]")]
		roomId, _ := strconv.Atoi( (room[ strings.LastIndex(room, "-") +1: strings.Index(room, "[")]) )
		roomName := strings.Replace(room[:strings.LastIndex(room,"-")],"-","",-1)
		roomName2 := strings.Replace(room[:strings.LastIndex(room,"-")],"-"," ",-1)
		//fmt.Println(roomCheck);
		//fmt.Println(roomId);
		//fmt.Println(roomName);
		for i:=1; i < len(roomCheck); i++{
			a := strings.Count(roomName, string(roomCheck[i -1]))
			b := strings.Count(roomName, string(roomCheck[i]))
			if (a != 0 && b != 0 && a > b || (a == b && roomCheck[i-1] < roomCheck[i]) ){
				nbRoomCheck = true
			}else{
				nbRoomCheck = false
				break
		...
		if nbRoomCheck == true{
			fmt.Println(roomName2)
			caesarIndex :=  roomId % 26
			fmt.Println(roomId, alphaIndex)
			caesarTrans := ""
			for i:=0; i < len(roomName2); i++{
				if (string(roomName2[i]) != " "){
					alphaIndex = caesarIndex + strings.Index(alpha, string( roomName2[i] ) )
					alphaIndex = alphaIndex % 26
					fmt.Println(string( roomName2[i] ), string(alpha[alphaIndex]), alphaIndex, strings.Index(alpha, string( roomName2[i] )) )
					caesarTrans += string(alpha[alphaIndex])
				}else{
					caesarTrans += " "
				}
			}
			fmt.Println(caesarTrans)
			nbRoom += roomId
		}
		//for _, roomName range roomName
